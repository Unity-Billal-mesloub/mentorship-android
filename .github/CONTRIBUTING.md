...
* Follow our [Commit Message Style Guide](https://github.com/Unity-Billal-mesloub/mentorship-android/wiki/Commit-Message-Style-Guide) when you commit your changes.
...
* Please consider raising an issue before submitting a pull request (PR) to solve a problem that is not present in our [issue tracker](https://github.com/Unity-Billal-mesloub/mentorship-android/issues). This allows maintainers to first validate the issue you are trying to solve and also reference the PR to a specific issue.
...
* When adding a screenshot, ensure that it can be viewed properly on all device. To resize the image use `<img>` tag with `src`, `height` and `width` attributes. Learn more [here](https://github.community/t/change-img-size/125949). It is suggested to set either width or height, not both, so as to maintain the aspect ratio.
* When adding multiple screenshots or list of key-value pairs, try using tables. You can use [this](https://www.tablesgenerator.com/markdown_tables) markdown table generator.
